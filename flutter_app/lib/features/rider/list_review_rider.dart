// features/rider/list_review_rider.dart
...
class _RiderReviewEntry {
  final OrderModel order;
  final ReviewSubmitModel? review;

  _RiderReviewEntry({required this.order, required this.review});

  int get rating => review?.riderrating ?? 0;
}

class ListReviewRider extends StatefulWidget {
  const ListReviewRider({super.key});

  @override
  State<ListReviewRider> createState() => _ListReviewRiderState();
}

class _ListReviewRiderState extends State<ListReviewRider> {
  ...
  List<_RiderReviewEntry> _allReviews = [];
  ...
  Future<void> _fetchAllReviews() async {
    ...
    try {
      final String studentId = GlobalData.usernameRider;
      ...
      final List<_RiderReviewEntry> entries = [];
      for (final order in orders) {
        ...
        entries.add(_RiderReviewEntry(order: order, review: review));
      }
  ...
  List<_RiderReviewEntry> get _filteredReviews {
  ...
  Widget _buildChip({
    ...
    final Color bg = isSelected ? _primary : baseColor.withOpacity(0.12);
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            if (icon != null) ...[
              Icon(icon, size: 16, color: isSelected ? Colors.white : fg),
  ...
  Widget _buildReviewCard(_RiderReviewEntry entry) {
    ...
    // 🎯 ดึงชื่อรายการอาหารมาแสดง (แบบเดียวกับหน้า ReviewRestaurant)
    String itemsText = "";
    ...
    final String comment =
        (review?.commentrider != null && review!.commentrider!.isNotEmpty)
        ? review.commentrider!
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 13.5,
              color:
                  (review?.commentrider != null &&
                      review!.commentrider!.isNotEmpty)
                  ? Colors.black87
                  : Colors.grey,
              height: 1.4,
            ),
